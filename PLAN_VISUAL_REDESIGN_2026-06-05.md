...
| F7 | Conocimiento — cards + buscador | `[x] APROBADA` | Cerrada 2026-06-08. Hero `rd-hero-gradient`, tabs píldora, wrapper `rd-canvas rd-section`. Inner cards (flujo, plataformas, OER, plantillas, empty states) a `.rd-card-grad-violet`. Emojis→Lucide (mapa `EMOJI_TO_LUCIDE`+`lucideName()`, también iconos del CMS). Chips de sector/nivel/estado/tipo/ruta unificados a Blue/Purple/Beige. Tipografía: cuerpos min `text-base`, chips/labels a `text-sm`, solo controles de paginación en `text-xs`. |
| F8 | Red — cards socios/stakeholders; hélice intacta | `[x] APROBADA` | Cerrada 2026-06-08. Hero `rd-hero-gradient`, tabs píldora, wrapper `rd-canvas rd-section`. Cards socios/stakeholders + country grid + empty state → `.rd-card-grad-violet`; helix container beige + tiles grad-violet. **DECISIÓN_HUMANA: cuádruple hélice homogeneizada a paleta** — `CATEGORY_META` y el bloque hélice usan solo Blue/Purple (eliminados verde/rosa); estructura del modelo de 4 cuadrantes intacta. Formulario: ceja púrpura + cuerpo beige, botón azul (antes eu-orange). Iconos de sector corregidos (nts→briefcase, hou→home) + animación hover. Tipografía min `text-base`. |
| F9 | Marketplace — hero + tabs + filtros | `[x] APROBADA` | Cerrada 2026-06-08. Hero `rd-hero-gradient`, tabs píldora con iconos Lucide, wrapper `rd-canvas rd-section`, intro grad-violet y filtros beige/píldora. |
| F10 | Marketplace — cards (5 tipos) variante (a) | `[x] APROBADA` | Cerrada 2026-06-08. **Grid 3→2 columnas** (`md:grid-cols-2`, gap-6). **Sistema editorial `.rd-card-mp` + `.rd-card-mp-hover`** (redesign.css): superficie blanco → violeta-blanco tenue `#FBF7FF`, radio 1.75rem, sombra púrpura, lift al hover, `overflow:hidden`. **Patrón CEJA**: banda púrpura oficial `#5222B0` (`.rd-card-mp-ceja`) en la parte superior con el **título en blanco** (`.rd-card-mp-title`); orden reorganizado: **ceja(título) → chips/badges → resumen → callouts/meta → footer**. (Se descartaron el degradado beige y el acento superior lineal en iteraciones previas; `.rd-card-grad-beige` queda definida sin uso en Marketplace.) **Jerarquía interna**: callout destacado = `rd-card-grad-violet` + acento lateral azul (`border-l-4 border-l-eu-blue`); mini-meta secundaria = blanca con borde `border-eu-purple/15`. **Type badge sólido de marca** (`bg-eu-blue`/`bg-eu-purple` text-white) + status/extra outline `bg-white/70 text-eu-purple border-eu-purple/25`. **Tipografía editorial**: resumen y valores `text-base leading-relaxed`; mini-meta label `text-xs`. Footer: divisor púrpura, label entidad `text-eu-purple/60`. Chips/tonos unificados a Blue/Purple/Beige (SDG, competencias, verificación, evidencia, infra, pilotStage, pilotType, niveles, decisión, entornos). Commit recuperación `6f19f89`. |
| F11 | Revisión global + accesibilidad | `[x] APROBADA` | Cerrada 2026-06-08. Finalizado el protocolo P0–P7. Degradados unificados, iconos animados sobre blanco, eliminación de tokens legacy eu-orange/eu-teal. Mejoras de accesibilidad y responsive aplicadas. Removidas las restricciones max-w en párrafos de introducción de Conocimiento, Gobernanza, Marketplace y Red para que fluyan al ancho completo disponible. |
