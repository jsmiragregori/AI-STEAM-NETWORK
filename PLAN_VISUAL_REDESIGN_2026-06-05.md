...
| F10 | Marketplace — cards (5 tipos) variante (a) | `[~] EN_REVISION` | Iniciada 2026-06-08, **rediseño v2** (el degradado beige se descartó por verse sucio y chocar con el grad-violet interno). **Grid 3→2 columnas** (`md:grid-cols-2`, gap-6). **NUEVO sistema editorial `.rd-card-mp` + `.rd-card-mp-hover`** (redesign.css): superficie blanco → violeta-blanco muy tenue `#FBF7FF`, **franja de acento superior de 4px en degradado de marca (Electric Blue→Deep Purple)**, radio 1.75rem, sombra suave púrpura, lift al hover. Evita blanco plano y beige sucio. (`.rd-card-grad-beige` queda definida pero sin uso en Marketplace.) **Jerarquía interna**: callout destacado = `rd-card-grad-violet` + borde + acento lateral azul (`border-l-4 border-l-eu-blue`), label en `text-eu-blue`; mini-meta secundaria = blanca con borde `border-eu-purple/15` para contraste limpio sobre el outer. **Type badge sólido de marca** (`bg-eu-blue`/`bg-eu-purple` text-white) + status/extra badges outline `bg-white/70 text-eu-purple border-eu-purple/25`. **Título** en Deep Purple `text-xl font-extrabold`. **Tipografía editorial**: resumen y valores callout/mini-meta `text-base leading-relaxed`; mini-meta label `text-xs`. Footer: divisor púrpura, label entidad `text-eu-purple/60`. Chips/tonos unificados a Blue/Purple/Beige (SDG, competencias, verificación, nivel evidencia, infra, pilotStageTone, pilotType, LEVEL_STYLES FP, decisión validación, entornos). `npm run build:css` OK, `node --check` OK, `:3000`→200. **Pendiente revisión visual humana.** |
